...
| Hermes household profiles | **Very strong gated design** | Separate OpenShell sandboxes, profile credentials, `owner_scope`, external approval, snapshots, 64K context, and cross-profile negative tests are in URS-PA-002–014 and Phases I/J. | Choose the always-on application host, deploy one synthetic `owner` pilot, then add `partner`. Hermes profiles alone are not OS/filesystem isolation; keep the separate-sandbox rule. Hermes supports local OpenAI-compatible endpoints and stores memory locally. [Hermes FAQ](https://github.com/NousResearch/hermes-agent/blob/main/website/docs/reference/faq.md), [NemoClaw overview](https://docs.nvidia.com/nemoclaw/user-guide/hermes/home). |
...
- Enable SSRF protection on a supported n8n release and narrowly allow-list the
  internal `ai.home`, Home Assistant, and approved MCP hosts. n8n's default SSRF
