...
| Specialist local LLM | Repository implementation or a difficult, bounded research/verifier job | Always-on use or concurrent bulk work by assumption | Qwen3.8-27B as the first quality candidate; Nemotron 3.5 Lightning as the performance candidate; Ornith only if a measured coding gap remains |
...
| Hermes | Retrieval below principal/data-domain/visibility scope; application sessions remain outside GB10 | `assistant` → shared Qwen3.6 at a separately qualified 64K context | Interactive on demand; at most 1–2 useful briefings/profile/day; stagger profile schedules and dedupe family overlap |
