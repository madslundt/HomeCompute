        ...
        three separately isolated Hermes runtimes
             owner | partner | family
              ...
              OpenShell inference.local
                         |
                         v
       existing ai.home edge + qualified LiteLLM
                         |
                         v
...
| --- | --- | --- |
| `owner` | `owner_scope IN ('owner','shared')` | Partner-private credentials/data |
| `partner` | `owner_scope IN ('partner','shared')` | Owner-private credentials/data |
| `family` | `owner_scope = 'shared'` | Either person's private email, banking, recordings, or private conversations |
...
3. Reuse the existing `ai.home` model path. Qualify Hermes first against direct
...
- Demonstrate adversarial cross-profile tests for Discord identity routing,
  API keys, memory/session search, database `owner_scope`, MCP credentials, and
